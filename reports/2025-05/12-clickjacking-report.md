# Technical Pentest Report — Lab 12 Clickjacking
...
**Primary Weakness:** Missing Frame Embedding Protections  
**Severity:** Medium  
...
## 4. Finding Overview
### Missing Frame Embedding Protections
**Severity:** Medium  
**Category:** Browser Security Controls

## 5. Description
The vulnerable version allowed behavior associated with **Missing Frame Embedding Protections**. The application trusted data, state, or rendering paths in a way that introduced a security weakness. In the context of the lab, this behavior was sufficient to demonstrate the core vulnerability pattern and its practical impact.

## 6. Technical Details
The page lacked controls preventing untrusted framing. This allows a malicious site to visually disguise the victim application and induce sensitive clicks under false pretenses.

## 7. Proof of Concept / Test Input
```text
Conceptual iframe overlay attack scenario
```

## 8. Security Impact
Affected workflows may include approvals, transfers, configuration changes, or any action that should require deliberate user intent.
...
Use frame-ancestors in CSP, consider X-Frame-Options for compatibility, and add additional friction for high-risk state-changing actions.
...
This simulated assessment confirmed the presence of **Missing Frame Embedding Protections** in the vulnerable version and validated the intended mitigation approach in the fixed version. The lab effectively demonstrates both the offensive behavior and the secure development correction pattern.
...
### Review Note\nReport expanded to better connect framing abuse with sensitive user actions.
